...
// ─── 2. Permissions (keyed by group name) ───────────────────────────────────
...
async function seedPermissionGroups(): Promise<Map<string, string>> {
...
async function seedPermissions(groupMap: Map<string, string>): Promise<Map<string, string>> {
  const map = new Map<string, string>();
...
async function seedRoles(permMap: Map<string, string>): Promise<Map<string, string>> {
  const roleIdMap = new Map<string, string>();

  // Pass 1: upsert roles without hierarchy
...
async function seedSuperAdminUser(roleIdMap: Map<string, string>): Promise<void> {
...
// ─── 5. Test Users (All 8 from TEST_RESULTS.md) ────────────────────────────
...
async function seedTestUsers(roleIdMap: Map<string, string>): Promise<void> {
  // Seed 6 admin users
  for (const admin of TEST_ADMIN_USERS) {
    ...
    const user = await prisma.user.upsert({
      ...
      update: { email: admin.email, passwordHash, userType: UserType.ADMIN },
      create: { phone: admin.phone, email: admin.email, passwordHash, userType: UserType.ADMIN },
    });
  ...
  // Seed client user + tenant + profile + role
  {
    const c = TEST_CLIENT;
    ...
    const user = await prisma.user.upsert({
      where: { phone: c.phone },
      update: { email: c.email, passwordHash, userType: UserType.CLIENT },
      create: { phone: c.phone, email: c.email, passwordHash, userType: UserType.CLIENT },
    });
  ...
  // Seed SP user + profile + role
  {
    ...
    const user = await prisma.user.upsert({
      where: { phone: s.phone },
      update: { email: s.email, passwordHash, userType: UserType.SP },
      create: { phone: s.phone, email: s.email, passwordHash, userType: UserType.SP },
    });
